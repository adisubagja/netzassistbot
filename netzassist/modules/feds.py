...
from netzassist import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, TEMPORARY_DATA, LOGGER, spamcheck
from netzassist.modules.helper_funcs.chat_status import is_user_admin
from netzassist.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from netzassist.modules.helper_funcs.string_handling import markdown_parser
from netzassist.modules.disable import DisableAbleCommandHandler
 
import netzassist.modules.sql.feds_sql as sql
...
def new_fed(update, context):
    ...
    if not fednam == '':
        ...
        elif fednam == "netzassist Official":
            fed_id = "netzassistOfficial"
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "netzassist_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="netzassist_fbanned_users.json",
                    caption=tl(
                        update.effective_message,
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "netzassist_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="netzassist_fbanned_users.csv",
                    caption=tl(
                        update.effective_message,
                        "Total {} pengguna terkena blokir federasi {}.").format(
                        len(getfban),
