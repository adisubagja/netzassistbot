...
from netzassist import dispatcher, spamcheck, LOGGER
from netzassist.modules.disable import DisableAbleCommandHandler
from netzassist.modules.helper_funcs.chat_status import user_admin_no_reply, user_admin

from netzassist.modules.sql import languages_sql as sql
...
for x in os.listdir('netzassist/modules/langs'):
	if os.path.isdir('netzassist/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("netzassist.modules.langs." + x)
